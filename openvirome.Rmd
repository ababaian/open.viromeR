...
params:
  ov.version: '0.0.8'
  analysis_name: 'Apicomplexa'
  search_type: "LIST"            # "LIST" or "STAT" or "SEARCH"
  virome_search_term: '%%'  # Scientific_name search term
  virome_deplete_term: '' # Deplete this term from matches, or '' for none
  input.path: inst/extdata/apicomplexa # <prefix>.stat OR <prefix>.list
...
p$output.html   <- paste0(p$analysis_name, p$report_id, '.html')
p$output.rdata  <- paste0(p$analysis_name, p$report_id, '.RData')
...
# Print out version
# TODO Merge the print output of parameters with the generation of the parameters above
...
```{r,  fig.height = 4, fig.width = 8, retina = 3}
plot.virFam.n
```
...
```{r,  fig.height = 6, fig.width = 10, retina = 3}
# Virome Family count bar plot
